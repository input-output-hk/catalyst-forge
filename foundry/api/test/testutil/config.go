...
func DefaultTestConfig() (*Config, error) {
    ...
    return &Config{
        HTTPPort:          port,
        PublicBaseURL:     fmt.Sprintf("http://127.0.0.1:%d", port),
        RefreshHashSecret: getStringEnv("TEST_REFRESH_HASH_SECRET", func() string { return RandomHex(16) }),
        RateLimitBurst:    parseIntEnv("TEST_AUTH_RATE_BURST", 100),
        RateLimitWindowSec: parseIntEnv("TEST_AUTH_RATE_WINDOW_SEC", 60),
        BootstrapToken: getStringEnv("TEST_BOOTSTRAP_TOKEN", func() string {
            // Ensure length >= 32 to satisfy config validation
            return RandomHex(24)
        }),
    }, nil
}
