...
func TestAuth_Onboard_Begin_And_Complete(t *testing.T) {
	...
	resp, err := tu.DoJSON(nil, http.MethodPost, env.BaseURL()+"/api/v1/auth/onboard/begin", nil, map[string]any{
	...
	resp, err = tu.DoJSON(nil, http.MethodPost, env.BaseURL()+"/api/v1/auth/onboard/begin", headers, map[string]any{
		"invite_id":   "00000000-0000-0000-0000-000000000000",
		"token":       "invalid",
		"device_name": "test",
	}, &begin)
	...
	resp, err = tu.DoJSON(nil, http.MethodPost, env.BaseURL()+"/api/v1/auth/onboard/complete", nil, map[string]any{
		"session_key": "bad",
