...
import (
	...
	"github.com/input-output-hk/catalyst-forge/foundry/api/internal/models/argo"
	base "github.com/input-output-hk/catalyst-forge/foundry/api/internal/repository"
)

// Repository defines the interface for argo sync operations
type Repository interface {
	Create(ctx context.Context, sync *argo.ArgoSync) error
	List(ctx context.Context, filter ListFilter) ([]argo.ArgoSync, int64, error)
	GetLatestByApp(ctx context.Context, envID uuid.UUID, appName string) (*argo.ArgoSync, error)
}

// ListFilter contains filter parameters for listing argo syncs
...
// Create creates a new argo sync record
func (r *repositoryImpl) Create(ctx context.Context, sync *argo.ArgoSync) error {
...
// List retrieves argo sync records with filters and pagination
func (r *repositoryImpl) List(ctx context.Context, filter ListFilter) ([]argo.ArgoSync, int64, error) {
	db := base.GetDB(ctx, r.db)
	
	query := db.Model(&argo.ArgoSync{})
...
func (r *repositoryImpl) GetLatestByApp(ctx context.Context, envID uuid.UUID, appName string) (*argo.ArgoSync, error) {
	db := base.GetDB(ctx, r.db)
	
	var sync argo.ArgoSync
