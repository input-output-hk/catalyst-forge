...
func (h *EnvironmentHandler) GetByProjectAndName(c *gin.Context) {
	var param contracts.EnvironmentNameParam
	if err := c.ShouldBindUri(&param); err != nil {
		h.RespondWithValidationError(c, err)
		return
	}

	// The service only supports GetByName, not by project
	...
	h.RespondWithSuccess(c, http.StatusOK, h.toResponse(env, param.ProjectID.String(), ""))
}
...
func (h *EnvironmentHandler) Update(c *gin.Context) {
	...
	env, err := h.service.Update(c.Request.Context(), param.EnvironmentID, svcReq)
	if err != nil {
		if errors.Is(err, environmentService.ErrEnvironmentNotFound) {
			h.RespondWithNotFound(c, "Environment")
			return
...
func (h *EnvironmentHandler) Delete(c *gin.Context) {
	...
	if err := h.service.Delete(c.Request.Context(), param.EnvironmentID); err != nil {
		if errors.Is(err, environmentService.ErrEnvironmentNotFound) {
			h.RespondWithNotFound(c, "Environment")
			return
		}
