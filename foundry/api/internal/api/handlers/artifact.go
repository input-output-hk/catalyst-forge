...
func (h *ArtifactHandler) GetByID(c *gin.Context) {
	var param contracts.ArtifactIDParam
	if err := c.ShouldBindUri(&param); err != nil {
		h.RespondWithValidationError(c, err)
		return
	}

	art, err := h.service.GetByID(c.Request.Context(), param.ArtifactID)
	if err != nil {
...
func (h *ArtifactHandler) Update(c *gin.Context) {
	...
	existing, err := h.service.GetByID(c.Request.Context(), param.ArtifactID)
	if err != nil {
		if errors.Is(err, artifactService.ErrArtifactNotFound) {
			h.RespondWithNotFound(c, "Artifact")
			return
	...
	art, err := h.service.Update(c.Request.Context(), param.ArtifactID, svcReq)
	if err != nil {
		if errors.Is(err, artifactService.ErrArtifactNotFound) {
			h.RespondWithNotFound(c, "Artifact")
			return
...
func (h *ArtifactHandler) Delete(c *gin.Context) {
	...
	if err := h.service.Delete(c.Request.Context(), param.ArtifactID); err != nil {
		if errors.Is(err, artifactService.ErrArtifactNotFound) {
			h.RespondWithNotFound(c, "Artifact")
			return
		}
