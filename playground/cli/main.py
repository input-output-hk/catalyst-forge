...
Usage examples:
    uv run python playground/cli/main.py k3d --name forge --servers 1 \
        --agents 0 --kubeconfig-out playground/kubeconfig \
        --output-json playground/cluster.json --yes
"""
...
__all__: list[str] = ["ClusterSummary", "k3d_up", "app"]
...
def dns_pin_registry(
    ...
    if any(h in corefile for h in host):
        ...
        for ln in corefile.splitlines():
            ...
            if (
                any(h in ln for h in host)
                and ln.strip().split()[0].replace(".", "").isdigit()
            ):
                continue
            pruned.append(ln)
        cm.data["Corefile"] = "\n".join(pruned)
        ...
        if in_main:
            ...
            if brace_depth == 0:
                ...
                if insert_at > 0 and lines[insert_at - 1].strip() != "":
                    managed_block = "\n" + managed_block
                new_lines = (
                    lines[:insert_at] + [managed_block.rstrip("\n")] + lines[insert_at:]
                )
                cm.data["Corefile"] = "\n".join(new_lines) + "\n"
                v1.patch_namespaced_config_map("coredns", "kube-system", cm)
...
def _generate_assets(
    *,
    ...
    Args:
        base_dir: The base directory of the playground (playground root).
    ...
    client = get_client_cert_paths(repo_root / "playground/.certs", client_name)
...
def k3d_up(
    ...
    https_port: int = typer.Option(
        443, min=1, max=65535, help="Host HTTPS port => LB:443"
    ),
    api_port: int = typer.Option(
        0, min=0, max=65535, help="Host API port for kube-apiserver (0=disabled)"
    ),
    kubeconfig_out: Path = typer.Option(
        (BASE_DIR / "kubeconfig"), help="Path to write kubeconfig"
    ),
    output_json: Path = typer.Option(
        (BASE_DIR / "cluster.json"), help="Path to write cluster summary JSON"
    ),
    force_recreate: bool = typer.Option(
        False, help="Delete and recreate cluster if it exists"
    ),
    yes: bool = typer.Option(
        False, "--yes", help="Assume 'yes' for prompts and overwrites"
    ),
    registry_host: str = typer.Option(
        "registry.projectcatalyst.dev",
        help="Registry host DNS name to trust via mkcert CA",
    ),
    ...
    if generate_client_cert:
        log(
            "[Deprecated] --generate-client-cert is deprecated; use 'generate' command instead."
        )
        _generate_assets(
            base_dir=BASE_DIR,
...
def deploy(
    ...
    deploy_service(
        service_name=service, kubeconfig_opt=kubeconfig, show_manifest=not no_manifest
    )
