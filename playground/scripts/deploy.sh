...
Environment variables:
  ...
  KUBECONFIG    Override kubeconfig path (defaults to playground/kubeconfig)
EOF
}
...
if [[ ! -f "${KCFG}" ]]; then
  err "Kubeconfig not found at '${KCFG}'. Run 'uv run python -m playground.cli.main k3d --yes' first."
  exit 1
fi
