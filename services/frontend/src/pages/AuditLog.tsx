...
const AuditLog = () => {
  ...
  const helmet = usePageTitle(
    "Audit Log – Catalyst Forge",
    "Filterable timeline of actions.",
    "/audit"
  );
  ...
  const filtered = useMemo(
    () =>
      rows.filter(
        (e) =>
          (!actor || (e.actor_id || "").includes(actor)) &&
          (!types || types.split(",").some((t) => e.type.includes(t.trim())))
      ),
    [rows, actor, types]
  );
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        type AuditResponse =
          paths["/api/v1/admin/audit"]["get"]["responses"][200]["content"]["application/json"];
