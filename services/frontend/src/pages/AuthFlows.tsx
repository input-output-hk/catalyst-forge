...
const AuthFlows = () => {
  ...
  const helmet = usePageTitle(
    "Auth Flows – Catalyst Forge",
    "Click-through demo of auth scenarios.",
    "/auth-demo"
  );
    ...
    <section className="container py-8">
      ...
      <Step title="Brand‑new user onboarding">
        <Button
          ...
          onClick={() =>
            setMessage(
              "Account created → email verified → WebAuthn credential registered → landing on Dashboard"
            )
          }
        >
          Start
      ...
      <Step title="Add new credential">
        <Button onClick={() => setMessage("Prompted for device → credential added")}>
          Add credential
        </Button>
      </Step>
      <Step title="Normal session">
        <Button
          variant="secondary"
          onClick={() => setMessage("Session valid; navigating around shows authed state")}
        >
          Show
        </Button>
      </Step>
      <Step title="Access‑token refresh">
        <Button
          variant="outline"
          onClick={() => setMessage("Token expired → silent refresh → last action retried")}
        >
          Simulate
        </Button>
      </Step>
      ...
      <Step title="Account recovery">
        <Button
          variant="outline"
          onClick={() => setMessage("Identity proofed → new credential registered")}
        >
          Recover
        </Button>
      </Step>
      <Step title="Forced re‑login">
        <Button
          variant="destructive"
          onClick={() => setMessage("Session invalidated → CTA to sign in again")}
        >
          Force
        </Button>
      </Step>
      <Step title="Logout / Logout all">
        <div className="flex gap-2">
          <Button
            onClick={() => {
              actions.logout();
              setMessage("Logged out on this device");
            }}
          >
            Logout
          </Button>
          <Button variant="outline" onClick={() => setMessage("All sessions cleared")}>
            Logout all
          </Button>
        </div>
      </Step>
