...
func (h *ProjectHandler) GetByID(c *gin.Context) {
	...
	id, err := h.ParseUUID(p.ProjectID)
	if err != nil {
		h.RespondWithValidationError(c, err)
		return
	}
...
func (h *ProjectHandler) GetByRepoAndPath(c *gin.Context) {
	...
	repoUUID, err := h.ParseUUID(up.RepoID)
	if err != nil {
		h.RespondWithValidationError(c, err)
		return
	}

	proj, err := h.service.GetByRepoAndPath(c.Request.Context(), repoUUID, qp.Path)
	if err != nil {
		if errors.Is(err, projectService.ErrProjectNotFound) {
			h.RespondWithNotFound(c, "Project")
			return
