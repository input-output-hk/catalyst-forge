...
func (f *failingAuditStore) GetEvents() []domain.Event { return f.events }

func (f *failingAuditStore) List(_ context.Context, actorID *uuid.UUID, userID *uuid.UUID, types []string, since *time.Time, until *time.Time, limit, offset int) ([]domain.Event, int64, error) {
	// Simple passthrough for tests; no filtering necessary for current usage
	total := int64(len(f.events))
