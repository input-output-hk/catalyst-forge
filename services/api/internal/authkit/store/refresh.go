...
type RefreshStore interface {
	...
	ListActiveByUser(ctx context.Context, userID uuid.UUID, now time.Time) ([]*domain.RefreshToken, error)

	// GetAnyByFamily retrieves any token from a family (for ownership checks, etc.).
	GetAnyByFamily(ctx context.Context, familyID uuid.UUID) (*domain.RefreshToken, error)
}
