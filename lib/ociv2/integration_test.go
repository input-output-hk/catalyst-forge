...
func TestIntegrationPushPull(t *testing.T) {
	...
	t.Run("JSON_Artifact_RoundTrip", func(t *testing.T) {
		...
		// Note: Resolve returns the manifest descriptor, not the config descriptor
		resolveDesc, err := client.Resolve(ctx, testRef)
		require.NoError(t, err)
		assert.NotEmpty(t, resolveDesc.Digest)
		assert.Contains(t, resolveDesc.MediaType, "manifest")

		// Test head operation
		// Note: Head returns the manifest descriptor, not the config descriptor
		headDesc, err := client.Head(ctx, testRef)
		require.NoError(t, err)
		assert.NotEmpty(t, headDesc.Digest)
		assert.Equal(t, resolveDesc.Digest, headDesc.Digest)
	})
