...
func NewDefaultCSRF(cfg CookieConfig, ttl time.Duration) *DefaultCSRF {
	// Enforce that SameSite is Strict for this mode
...
// The token is only for API compatibility; validation is header-only
func (c *DefaultCSRF) Generate() (string, error) {
	// Return a static token since we don't validate it
	// This avoids unnecessary randomness generation
	return "header-only-csrf-protection", nil
}

// Validate checks for valid CSRF protection
func (c *DefaultCSRF) Validate(r *http.Request) error {
	...
	// For refresh endpoint, we require the header value to be "XMLHttpRequest"
	if header != "XMLHttpRequest" {
		return ErrCSRFHeaderMissing
	}
	
	// Additional validation could check cookie vs header token match
	// but the custom header alone provides strong CSRF protection
	// since cross-origin requests cannot set custom headers without CORS
	
	return nil
}

// SetCookie is a no-op for DefaultCSRF since it's header-only protection
// The method exists only for interface compatibility
func (c *DefaultCSRF) SetCookie(w http.ResponseWriter, token string) {
	// No-op: DefaultCSRF is header-only and doesn't use cookies
	// This prevents confusion and token fixation risks
}
...
func NewDoubleSubmitCSRF(cfg CookieConfig, ttl time.Duration) *DoubleSubmitCSRF {
	// Generate a random secret for HMAC if not provided
	secret := make([]byte, 32)
...
func (c *DoubleSubmitCSRF) Generate() (string, error) {
	...
	// Create token data: nonce || expiry
...
func (c *DoubleSubmitCSRF) Validate(r *http.Request) error {
	...
	// Constant-time comparison of raw tokens
	...
	// Token must be exactly: 24 (nonce) + 8 (expiry) + 32 (hmac) = 64 bytes
	// Exact length check prevents any tampering with additional data
	if len(tokenBytes) != 64 {
		return ErrCSRFTokenInvalid
	}
	
	// Extract parts (safe now that we know exact length)
	nonce := tokenBytes[:24]
...
func (n *NoOpCSRF) SetCookie(w http.ResponseWriter, token string) {}
...
func NewMemoryCSRF(cfg CookieConfig, ttl time.Duration) *MemoryCSRF {
	return &MemoryCSRF{
		...
		cleanupInterval: time.Minute, // Clean every minute
	}
}
...
func (m *MemoryCSRF) Generate() (string, error) {
	...
	// Clean expired tokens inline if enough time has passed
