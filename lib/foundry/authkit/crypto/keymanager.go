...
func (m *es256KeyManager) JWKS() interface{} {
	...
	for kid, pubKey := range m.publicKeys {
		// Convert public key to JWK format with proper padding
		jwk := map[string]interface{}{
			...
			"alg": "ES256",
			"crv": "P-256",
			"x":   base64.RawURLEncoding.EncodeToString(coordinateBytes(pubKey.X)),
			"y":   base64.RawURLEncoding.EncodeToString(coordinateBytes(pubKey.Y)),
		}
...
// coordinateBytes ensures the coordinate is exactly 32 bytes for P-256.
func coordinateBytes(n *big.Int) []byte {
	bytes := n.Bytes()
	if len(bytes) == 32 {
		return bytes
	}
