...
func (s *Store) CreateRole(ctx context.Context, role r.RoleDef) error {
	...
	m := Role{ID: rid, Slug: role.Slug, Name: role.Name, Description: role.Description, Version: role.Version}
	entries := make([]RoleEntry, 0, len(role.Entries))
	for _, e := range role.Entries {
		entries = append(entries, RoleEntry{ID: uuid.New(), RoleID: rid, Effect: string(e.Effect), Permission: string(e.Permission), ResourceType: e.ResourceType})
	}
...
func (s *Store) GetRole(ctx context.Context, slug string) (*r.RoleDef, error) {
	...
	d := r.RoleDef{ID: m.ID, Slug: m.Slug, Name: m.Name, Description: m.Description, Version: m.Version}
	for _, e := range es {
		d.Entries = append(d.Entries, r.RoleEntry{Effect: r.Effect(e.Effect), Permission: r.PermissionKey(e.Permission), ResourceType: e.ResourceType})
	}
	return &d, nil
}
...
func (s *Store) UpdateRole(ctx context.Context, role r.RoleDef) error {
	...
	existing.Name, existing.Description, existing.Version = role.Name, role.Description, role.Version
	entries := make([]RoleEntry, 0, len(role.Entries))
	for _, e := range role.Entries {
		entries = append(entries, RoleEntry{ID: uuid.New(), RoleID: existing.ID, Effect: string(e.Effect), Permission: string(e.Permission), ResourceType: e.ResourceType})
	}
	return s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
