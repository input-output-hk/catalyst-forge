...
func TestCondition_RequiresStepUp(t *testing.T) {
	...
	// Pass: step-up valid and within 5m window
	ctx := EvalContext{Now: now, StepUpValidUntil: now.Add(4 * time.Minute)}
	...
	ctx2 := EvalContext{Now: now}
	pass, _, err = cond.Evaluate(ctx2, map[string]any{"within": "5m"})
	require.NoError(t, err)
	assert.False(t, pass, "should fail when no step-up present")

	// Fail: window exceeded (remaining > within)
	ctx3 := EvalContext{Now: now, StepUpValidUntil: now.Add(10 * time.Minute)}
	...
	ctx4 := EvalContext{Now: now, StepUpValidUntil: now.Add(5 * time.Minute)}
