# Mock OIDC provider integration for Kratos (playgroundv2)
...
## Pre-reqs
- [ ] Envoy Gateway installed and reachable (already provisioned by playgroundv2).
...
Tasks:
- [ ] Create `playgroundv2/terraform/mock_oidc.tf` with:
...
Tasks (in `playgroundv2/terraform/kratos.tf`):
...
## 7) Test
- [ ] Apply Terraform: `terraform -chdir=playgroundv2/terraform init && terraform apply`.
- [ ] Verify mock OP:
