...
type AssignCmd struct {
	UserID    *string `short:"u" help:"The numeric ID of the user to assign (mutually exclusive with --user-email)."`
	UserEmail *string `short:"e" help:"The email of the user to assign (mutually exclusive with --user-id)."`
	RoleID    *string `short:"r" help:"The numeric ID of the role to assign (mutually exclusive with --role-name)."`
...
func (c *AssignCmd) assignUserToRole(cl client.Client) error {
	...
	fmt.Printf("Successfully assigned user ID %d to role ID %d\n", userID, roleID)
	return nil
}
