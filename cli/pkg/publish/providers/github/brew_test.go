...
func TestBrewDeployer_Deploy(t *testing.T) {
	tests := []struct {
		...
		{
			name: "brew publish with custom template URL",
			cfg: PublisherConfig{
				Prefix: "tool",
				Name:   "Tool",
				Brew: &BrewConfig{
		...
		{
			name: "brew publish with missing target branch",
			cfg: PublisherConfig{
				Prefix: "branch-test",
				Name:   "BranchTest",
				Brew: &BrewConfig{
		...
		{
			name: "brew publish with git template repository",
			cfg: PublisherConfig{
				Prefix: "app",
				Name:   "App",
				Brew: &BrewConfig{
    ...
    bin.install "{{ .BinaryName }}"
			...
			remote := &rm.GitRemoteInteractorMock{
				CloneFunc: func(s storage.Storer, worktree gb.Filesystem, o *gg.CloneOptions) (*gg.Repository, error) {
					...
					if tt.name == "brew publish with git template repository" && cloneCallCount == 0 {
  ...
  test do
    system "#{bin}/{{ .BinaryName }}", "--version"
			...
			// Create project
			projectName := "my-cli"
			if tt.name == "brew publish with custom template URL" {
				projectName = "tool"
			} else if tt.name == "brew publish with missing target branch" {
				projectName = "branch-test"
			} else if tt.name == "brew publish with git template repository" {
				projectName = "app"
			}
			p := project.Project{
				Name: projectName,
