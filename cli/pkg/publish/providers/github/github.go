...
func (r *Publisher) Publish() error {
	r.logger.Info("Running publish target", "project", r.project.Name, "target", r.publisher.Target, "dir", r.workdir)
	...
	if r.project.Tag == nil {
		return fmt.Errorf("cannot create a GitHub release without a git tag")
	}
	...
	if r.config.Brew != nil {
		...
		if err := r.brewDeployer.Deploy(releaseAssets); err != nil {
...
func (r *Publisher) run(path string) error {
...
func (r *Publisher) validateArtifacts(path string) error {
	for _, platform := range r.getPlatforms() {
