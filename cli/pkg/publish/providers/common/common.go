...
var ErrConfigNotFound = fmt.Errorf("publish config field not found")
...
// ParseConfig parses the configuration for the publish target.
func ParseConfig(p *project.Project, name string, config any) error {
    err := p.Raw().DecodePath(fmt.Sprintf("project.publishers.%s.config", name), &config)
